...
Write-Host " _______  _______  ______    _______  _______  *     *  _______  ___   *  *_   __ " -ForegroundColor Red
Write-Host "|       ||   *   ||    * |  |       ||       || | _ | ||       ||   | | ||  | |  |" -ForegroundColor Red
Write-Host "|  _____||  |_|  ||   | ||  |    *  ||   *   || || || ||  _____||   |_| ||  |_|  |" -ForegroundColor Red
Write-Host "| |_____ |       ||   |_||_ |   |_| ||  | |  ||       || |_____ |      _||       |" -ForegroundColor Red
Write-Host "|_____  ||       ||    **  ||    **_||  |_|  ||       ||_____  ||     |_ |_     _|" -ForegroundColor Red
Write-Host " _____| ||   *   ||   |  | ||   |    |       ||   *   | _____| ||    _  |  |   |  " -ForegroundColor Red
Write-Host "|_______||__| |__||___|  |_||___|    |_______||__| |__||_______||___| |_|  |___|" -ForegroundColor Red
...
if (-Not [System.IO.File]::Exists($path)) {
    Write-Host "Cannot find the log file! Make sure to open the wish history in the game first!" -ForegroundColor Red
...
if ($matches.Length -eq 0) {
    Write-Host "Cannot find the wish history url! Make sure to open the wish history in the game first!" -ForegroundColor Red
    return
}
